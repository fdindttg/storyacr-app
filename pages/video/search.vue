 ...
 * @LastEditor: FYT1996
 * @LastEditTime: 2023-12-02 17:34:07
	...
	<view class="page_content">
		<view class="head_content">
			...
			<view class="search_box">
				<u-search v-model="contents.keyword" :showAction="true" :actionText="$t('text.search')" :animation="true" :placeholder="$t('text.input.keyword')" @custom="searchHandle" @search="searchHandle"></u-search>
			</view>
		</view>
		<view class="main_content">
			<scroll-view style="height: 100%" :scroll-y="true" :refresher-enabled="true" :refresher-threshold="100" :refresher-triggered="refreshStatus" @refresherrefresh="refreshHandle" @scrolltolower="bottomHandle">
				<view class="info_box" v-if="!isSearch">{{ $t('text.recommend') }}</view>
				<view class="content_box">
					<view class="list" v-if="contents.list && contents.list.length">
						<view class="item item2" v-for="(lItem, lIndex) in contents.list" :key="lIndex" @click="openVideoDetail(lItem.id)">
							<view class="img">
					...
					<view class="nodata" v-if="!contents.list.length && contents.status == 'nomore'">
						<u-empty mode="data" :text="$t('loading.nodata')" icon="/static/images/nodata.png" />
					</view>
					<view class="liststatus" v-if="contents.list.length && isSearch">
						<u-loadmore :status="contents.status" :line="true" :loadmoreText="$t('loading.loadmore')" :loadingText="$t('loading.loading')" :nomoreText="$t('loading.nomore')" />
					</view>
				</view>
			</scroll-view>
		</view>
