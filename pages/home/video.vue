 ...
 * @LastEditor: FYT1996
 * @LastEditTime: 2024-01-03 16:50:37
				...
				<swiper-item class="swiper_item" v-for="(item, index) in videoData" :key="index">
					...
					<view class="infobox" v-if="!isDrag && videoIndex == index">
						...
						<view class="content">
							...
							<text class="text1">{{ item.name }} / {{ $t('video.total', item.video.episodes) }}</text>
							<text class="text2" style="margin-left: 20rpx;" @click="openVideoDetail(item.video.id)">{{ $t('video.more') }}</text>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			...mapGetters("app", ["adCountdown", "config", "lang"])
		},
		watch: {
		...
		methods: {
			...
			initSwiperData(originIndex = this.originIndex, init = 0) {
				...
				this.handleViews(this.videoData[this.videoIndex].vid, this.videoData[this.videoIndex].id)
			},
			// swiper切换
			swiperChange(event) {
			...
			handleViews(vid, id) {
				if(!vid || !id) return
				this.$request('video.views', { vid, episode_id: id }, false)
			}
		}
	}
</script>
