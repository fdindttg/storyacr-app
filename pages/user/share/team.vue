 ...
 * @LastEditor: FYT1996
 * @LastEditTime: 2023-12-04 18:19:57
...
<template>
	<view class="page_content">
		<view class="head_content">
			<CustomNavbar :title="$t('navbar.team')"></CustomNavbar>
		</view>
			...
			<scroll-view class="scroll_view" :scroll-y="true" @scrolltolower="scrollBottom">
				<view class="scroll_content">
					<view class="top_card" :style="'background:' + global.isBgColor">
						<view class="text1">{{ info.count }} {{ $t('team.people') }}</view>
						<view class="text2">{{ $t('team.direct') }} {{ info.count_direct }} {{ $t('team.people') }}</view>
						<view class="text2">{{ $t('team.indirect') }} {{ info.count_indirect }} {{ $t('team.people') }}</view>
					</view>
					<view class="content_box">
						<view class="title">{{ $t('team.info') }}</view>
						<view class="list_box" v-if="list.length">
							<view class="item" v-for="(item, index) in list" :key="index">
								...
								<view class="right" v-if="item.type == '1'">{{ $t('team.direct') }}</view>
								<view class="right" v-else>{{ $t('team.indirect') }}</view>
							</view>
						</view>
						<view class="be_empty" v-else>{{ $t('team.not.info') }}</view>
					</view>
				</view>
			</scroll-view>
		</view>
